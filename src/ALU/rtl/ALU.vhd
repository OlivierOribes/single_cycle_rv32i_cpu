...
begin
    ...
    begin
        ...
        case ALU_ctrl is
            ...
            when "0000" => result := '0' & (A and B);
            when "0001" => result := '0' & (A or  B);
            when "0011" => result := '0' & (A xor B);
            ...
            -- SHL : shift left logical by 1.
            -- Carry = evicted MSB (A(DATA_WIDTH-1)), result(DATA_WIDTH) = 0
            when "1101" =>
                result := '0' & std_ulogic_vector(shift_left(unsigned(A), 1));
                carry  := A(DATA_WIDTH-1);      -- evicted bit

            -- SHR : shift right logical by 1.
            -- Carry = evicted LSB (A(0)), result(DATA_WIDTH) = 0
            when "1110" =>
                result := '0' & std_ulogic_vector(shift_right(unsigned(A), 1));
                carry  := A(0);                 -- evicted bit
